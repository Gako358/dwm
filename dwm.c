...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen, CenterThisWindow, issticky;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0) {
        ...
        if (selmon->sel) {
            ...
            // Note: strtok() modifies selmon->sel->name in-place.
            // but that does not matter because we are about to exec()
            // below anyway; selmon->sel->name is not used again.
            while (cwd) {
                if (*cwd == '~') { // Replace ~ with $HOME
                    if (!(pathbuf = malloc(homelen + strlen(cwd)))) { // Counts for a null terminator.
                        die("fatal: could not allocate memory for pathbuf %u\n",
                            homelen + strlen(cwd));
                    }
...
{
	if (*altbarcmd)
        // if not system(altbarcmd) != -1 TODO: Warning: system() is deprecated.
		system(altbarcmd);
}
